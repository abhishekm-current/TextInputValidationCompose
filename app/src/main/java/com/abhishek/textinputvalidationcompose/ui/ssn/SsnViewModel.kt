...
class SsnViewModel(savedStateHandle: SavedStateHandle) : ViewModel() {
    val firstThree = FormTextFieldState(
        savedStateHandle,
        listOf(LengthValidator(3))
    )
    val nextTwo = FormTextFieldState(
        savedStateHandle,
        listOf(LengthValidator(2))
    )
    val lastFour = FormTextFieldState(
        savedStateHandle,
        listOf(LengthValidator(4))
    )
}
